...
import { rediscoverExternalSchema } from '@/lib/enterprise-ai-external-data'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const data = await rediscoverExternalSchema(access.tenantId)

    await logAudit({
      tenantId: access.tenantId,
      userId: access.userId,
      action: 'ENTERPRISE_AI_EXTERNAL_DATA_DISCOVER',
      entity: 'EnterpriseAiExternalDataConnection',
      newValues: {
        tableCount: data.tables.length,
        defaultTransactionReadOnly: data.defaultTransactionReadOnly,
      },
      req,
    })
    ...
    console.error('[ENTERPRISE AI EXTERNAL DATA DISCOVER]', err)
